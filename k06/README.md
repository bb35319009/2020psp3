...
## ソースコードの説明
75~ 動的計画法で1000円で最大取得可能なカロリーを求める。
75~79　表を初期化する
81~84　メニューを一つずつ容量の少ないところから順番に入れていき、メニューが入らないときは一つ上の値をコピーする。
86~92　メニューが入る場合は入れ替えた場合のカロリーの値と入れ替えなかった場合のカロリーの値(を比較し、大きいほうの値を入れる。
96　最大摂取カロリーを出力する。
