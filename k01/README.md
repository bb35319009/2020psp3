...
for と　if　を使い先頭から順番に値を一つ一つ調べていく。
一致しない場合は次の要素をチェックし、一致した場合はその要素が何番目かを返す。
全ての要素を調べても一致するものがない場合は-1を返す

int BinarySearch について

左から値の小さい順番に並べる。
範囲を中央で分割し、中央とkeyが一致するならそれを返す。
探す値の方が小さければ前に比較した要素の左側だけでリストを作る。
探す値の方が大きければ前に比較した要素の右側だけでリストを作る。
リストの中央とkeyが一致するならそれを返す。
一致するまでこれを繰り返す。
